...
const SignupSchema = Yup.object().shape({
  first_Name: Yup.string().required("First Name is required"),
  last_Name: Yup.string().required("Last Name is required"),
  description: Yup.string().when('$isSignup', {
    is: true,
    then: Yup.string().required("Description is required for signup")
  })
});
...
const AuthForm = () => {
  ...
  const handleSignupSubmit = async (values, { setSubmitting }) => {
    try {
      await axios.post("https://fruit-ai-backend-eigz.onrender.com/account/signup/", values);
      alert("Account created successfully!");
    } catch (error) {
      alert("Account creation failed: " + (error.response?.data?.detail || "Unknown error"));
    }
    setSubmitting(false);
  };
  ...
  const handleSigninSubmit = async (values, { setSubmitting }) => {
    try {
      await axios.post("https://fruit-ai-backend-eigz.onrender.com/account/signin/", values);
      alert("Sign-in successful!");
      navigate("/chat");
    } catch (error) {
  ...
  return (
    <div className="auth-container">
      <h2>{isSignup ? "Let's Get You Set Up" : "Log In"}</h2>
      <Formik
        initialValues={{ first_Name: "", last_Name: "", description: "" }}
        validationSchema={SignupSchema}
        ...
        onSubmit={isSignup ? handleSignupSubmit : handleSigninSubmit}
        context={{ isSignup }}  // Pass `isSignup` to Yup context
      >
        {({ errors, touched, isSubmitting }) => (
          <Form>
            <div>
